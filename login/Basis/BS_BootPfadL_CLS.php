...
final class PathHelper {
    ...
    public static function init(string $webBase, string $loginDir = 'login'): void {
        ...
        if ($docRoot === '' || $docRoot === 'DOCUMENT_ROOT') {
            // Fallback für CLI oder wenn DOCUMENT_ROOT nicht gesetzt ist
            $docRoot = dirname(__DIR__, 2); // Gehe von login/Basis/BS_BootPfadL_CLS.php zu FSArch_Hist
            // In CLI: webBase ist bereits im Pfad enthalten, also nicht hinzufügen
            $fsBase = $docRoot;
        } else {
            self::$documentRoot = rtrim($docRoot, "/\\");
        ...
        self::$urlLoginRoot = rtrim($webBase . '/' . $loginDir, '/');
        self::$inited = true;
    }

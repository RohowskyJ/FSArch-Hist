 ...
 *
 */
class FI_ListeRepository {
    private PDO $pdo;
    protected static string $logFile = 'FI_ListenRepository_debug.log.txt';
     ...
     * @return array
     */
    public function getFirmen(string $listType, ?string $search = null): array {
        // SQL 
        $sql = "
            SELECT
                fi_id,
    ...
    protected function modifyRow(array &$row, $tabTyp)
    {
        ...
        $fi_id = $row['fi_id'] ?? 0;
        $row['action'] = "<a href='VS_FirmenEdit.php?ID={$fi_id}'>Edit</a>";
